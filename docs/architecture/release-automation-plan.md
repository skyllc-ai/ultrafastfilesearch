...
3. Delete version-bump functions from
   ...
   - Keep `get_current_version()` and
     ...
     names).
   - `update_polars_git()` — removed 2026-06-09 (see §8.1
     "publishability resolution (crates.io polars 0.54)"); Polars
     is now a plain crates.io SemVer dependency with no git rev to
     bump, so the function and its `STEP_UPDATE_POLARS` ship step
     no longer exist.
...
- Trusted-publisher registration done for all 13 reserved crate
  names.
- `crates.io-publish` GitHub Environment exists with correct
  protection rules.
...
3. Add `[workspace.metadata.release-plz] publish = true` in
   ...
   publishing path.
4. Lock the publishing workflow behind the `crates.io-publish`
...
| Environment name | `crates.io-publish` | Must match the GHA `environment:` value in `release-plz.yml` exactly |
...
- [ ] `crates.io-publish` GitHub Environment exists
...
| R1b | Conventional commits (mandatory gate) | 🟢 | `HEAD` | 2026-06-10 | (this PR) | Promoted from R1a advisory after ~1.5 months observation (R1a landed 2026-04-25). Pre-flip verification: trailing **80 first-parent merges on `main` were 100% conformant** against the 11-standard-type pattern (the `security:`/`bench:`/`shmem:` prefixes from the R1a baseline converged out — `security` migrated to `fix(security):`/`chore(security):`, see the R1b CC-type convergence deviation row). `commitlint.yml` now `exit 1` (was `exit 0`) on non-conforming titles + posts the sticky comment. **Enforced**: `PR title — Conventional Commits` added to the `main-protection` ruleset's `required_status_checks` (ruleset id `11889528`, `strict` policy on) alongside `PR Fast CI / required`, so a non-conforming title now blocks merge. Caveat: the job is skipped on fork PRs (no write token for the sticky comment) — the normal UFFS flow (internal + Dependabot branches) is unaffected since those satisfy `head.repo.full_name == github.repository`; true external forks are guided via the PR template instead. |
...
| Path B gap — release PR merged, no binaries (auto-trigger added) | 2026-06-12 | First post-Path-B ship cycle (v0.5.121, PR [#395](https://github.com/skyllc-ai/UltraFastFileSearch/pull/395)) merged its release PR and produced **no binaries** — the operator expected artifacts ("it did so in the past"), but every release.yml run back through v0.5.114 was in fact `workflow_dispatch`; the Path B second step (`just release-tag`) was simply forgotten, and the ship pipeline's completion banner still promised "GitHub Actions will produce binaries" on merge. | Path B split the release into two manual steps but the ship UX never surfaced the second step prominently (one 💡 hint line above a wall of sccache stats), and the stale banner text actively claimed the opposite. A two-manual-step flow whose second step is easy to forget is a process bug, not an operator bug. | Added `.github/workflows/release-auto-trigger.yml`: on `pull_request: closed` into `main`, if merged AND head branch matches `release/v*`, dispatch release.yml for that version (`actions: write` only; GITHUB_TOKEN workflow_dispatch creates runs, unlike the R4 tag-push anti-loop case). Predicate keys on the branch name only `just ship`/`just release-pr` creates, so the Path B guarantee (normal merges trigger nothing) is preserved. `just release-tag` demoted to documented manual fallback (re-cuts / failed dispatch); `just/build.just` Path B comment block + `version-bump` hint updated. v0.5.121 itself was dispatched by hand (run 27422916912) before this landed. | `just ship` is end-to-end again: validate → bump → release PR → auto-merge → binaries, one command. The ship banner's promise is now true instead of stale. |
