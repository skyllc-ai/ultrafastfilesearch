...
| R5 | Retire bespoke tooling (incl. `scripts/ci/ci-pipeline.rs` thin wrapper per its `REMOVE-AFTER: v0.5.73` marker) | 🔴 ROLLBACK | `779c14fb1` (landed); reverted same-day | 2026-05-08 | [#153](https://github.com/skyllc-ai/UltraFastFileSearch/pull/153) (landed) → revert PR (this PR) | Landed via PR #153 then rolled back same-day pending polars-upstream chrono-compat release.  See `R5 rollback (polars-upstream wait)` deviation row in §8.  Re-application is a single-PR forward step once `cargo package -p uffs-polars` succeeds end-to-end. |
