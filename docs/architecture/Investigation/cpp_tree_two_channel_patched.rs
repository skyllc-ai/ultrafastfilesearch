...
impl<'a> CppTreeTraversal<'a> {
    ...
    fn preprocess(&mut self, idx: usize, name_info: u16, total_names: u16) -> PreprocessResult {
        ...
        if is_directory {
            // Channel B (printed): store metrics that correspond to the *directory stream only*.
            //
            // C++ propagates totals using the recursion return value (Channel A), which counts
            // *all* streams (including internal streams like $REPARSE_POINT, $ATTRIBUTE_LIST,
            // $SECURITY_DESCRIPTOR, ADS, etc.).
            //
            // But C++ prints directory metrics from the directory stream (type_name_id==0)
            // after it absorbs children_size.*. That printed view excludes the directory's own
            // non-directory streams. This “two-channel” behavior is required for 100% parity.
            ...
            // Files: printed size is the file's own size (all streams stored on the record).
            // Hardlink de-duplication happens via delta() in the propagated return value.
