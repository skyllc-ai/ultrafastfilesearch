...
**Scope** C: + D: + F: + G: NTFS volumes · 12 815 626 live file records (C/F NVMe, D HDD, G removable 16 GB test volume)
...
- **Full-scan export is a workload Everything does not run** in this harness (`es.exe` ~2 GB IPC export ceiling). UFFS streams the complete 10.21 M-row CSV in **4.8 s** (combined C+D+F+G) — **≈ 2.11 M records/sec**.
...
|-------|---------------------:|-----:|-----------:|
| C: | 1.56 s | 3 295 681 | 2.11 M rec/s |
| D: | 2.31 s | 4 772 520 | 2.07 M rec/s |
| F: | 0.96 s | 2 124 007 | 2.22 M rec/s |
| C+D+F+G: | **4.83 s** | **10 207 863** | **≈ 2.11 M rec/s** |

Sustained throughput through the daemon → CSV pipe is **≈ 2.11 M records/sec** (April snapshot: 1.72 M rec/s — **+23%**). `--hide-system --hide-ads` strips reserved NTFS metafiles and Alternate Data Streams to match Everything's default scope.
...
*Snapshot compiled 2026-06-11 from a single `just bench-suite` run (bundle `bench-20260611T221143Z-v0.5.120`). Raw data: [`raw/2026-06-v0.5.120_cross-tool-summary.csv`](raw/2026-06-v0.5.120_cross-tool-summary.csv) (every p50/p95/row count above) and [`raw/2026-06-v0.5.120_full-suite.csv`](raw/2026-06-v0.5.120_full-suite.csv). Numbers reflect UFFS v0.5.120 on the stated hardware and volumes.*
