...
if [ ! -f "$SOURCE_EXE" ]; then
    ...
    echo -e "${YELLOW}   Run this first: UFFS_PROFILING_BUILD=1 rust-script scripts/ci/build-cross-all.rs${NC}"
...
if [ -f "$SCRIPT_DIR/../windows/windows-profiling.ps1" ]; then
    cp "$SCRIPT_DIR/../windows/windows-profiling.ps1" "$DEST_DIR/run-profiling.ps1"
