// // Shared disk-level utilities (sector reading, caching, etc.)
//
...
//
// use crate::config::constants::{LOG_DATE_FORMAT, MAX_DIRS, MAX_DIRS_ALL,
// MAX_FILES_ALL};
// use crate::modules::directory_reader::{count_all_disk_entries,
// DirectoryReader, ReadDirectories4};
...
// use crate::modules::entities::disk::{DriveInfo, DriveType};
// use crate::modules::utils::format_utils::{format_number, format_size};
// use crate::modules::utils::time_utils::format_duration;
//
//
// pub async fn init_drives() -> Result<Vec<DriveInfo>, UFFSError> {
//     info!("got to: init_drives");
//
//     // Get the drive information
//     let drives_result = get_drive_info();
//
//     // Handle any errors from the get_drive_info function
//     let drives = drives_result?;
//
//     // Prepare a list of async tasks to fetch additional info for each drive
//     let mut tasks = vec![];
//
//     for mut disk in drives {
//         let task = task::spawn(async move {
//             // Fetch the number of files and directories for the current
// drive             let (num_files, num_dirs) =
// get_file_dir_len(&disk.root_path).await.map_err(|e| {
// UFFSError::DirectoryReadError // Convert the error type             })?;
//
//             // Update only the fields that need to be updated (num_files and
// num_dirs)             disk.num_files = num_files;
//             disk.num_dirs = num_dirs;
//
//             // Return the updated DriveInfo
//             Ok(disk) as Result<DriveInfo, UFFSError>
//         });
//
//         tasks.push(task);
//     }
...
//         .iter()
//         .map(|path| {
//             let path = path.clone(); // Clone the PathBuf to extend its
// lifetime             let disk_info = disk_info.clone();
//             let directory_reader_clone =
// Arc::clone(&current_directory_reader);             task::spawn(async move {
//                 let (files_all, dirs_all, duration, formatted_duration,
// disk_type, disk_size) =                     list_files_and_dirs(path.clone(),
// &disk_info, directory_reader_clone).await;                 (
//                     path,
//                     files_all.len(),
//                     dirs_all.len(),
//                     duration,
...
//
//     // Process results
//     for (path, files_len, dirs_len, duration, formatted_duration, disk_type,
// disk_size) in         results_futures.into_iter().filter_map(Result::ok)
//     {
...
//     // Header
//     println!(
//         "{:<path_length$} {:<type_length$} {:>size_length$} {:>files_length$}
// {:>dirs_length$} {:>time_seconds_length$} {:>time_length$}",         "Path".
// bold().underline().blue(),         "Type".bold().underline().blue(),
...
//
//     // Data rows
//     for (path, disk_type, size, files, dirs, duration, formatted_duration) in
// results {         println!(
//             "{:<path_length$} {:<type_length$} {:>size_length$}
// {:>files_length$} {:>dirs_length$} {:>time_seconds_length$.3}
// {:>time_length$}",             path,
//             disk_type,
//             format_size(size),
...
//     println!(
//         "{:<path_length$} {:<type_length$} {:>size_length$} {:>files_length$}
// {:>dirs_length$} {:>time_seconds_length$.3} {:>time_length$}",
//         "Total".bold().yellow(),
//         "",
...
// pub(crate) async fn get_file_dir_len(root_path: &PathBuf) -> Result<(u64,
// u64), UFFSError> {     let start = Instant::now();
