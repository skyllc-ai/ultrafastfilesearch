...
pub struct Win32LogicalDisk {
    ...
    /// The `DeviceID` property contains the unique identifier for the logical
    /// disk.
    DeviceID: String,

    /// The `DriveType` property indicates the type of disk drive (e.g., local
    /// disk, network drive).
    DriveType: Option<u32>,

    /// The `FileSystem` property specifies the file system used by the disk
    /// (e.g., NTFS, FAT32).
    FileSystem: Option<String>,

    /// The `FreeSpace` property indicates the available free space on the
    /// logical disk, in bytes.
    FreeSpace: Option<u64>,

    /// The `Size` property indicates the total size of the logical disk, in
    /// bytes.
    Size: Option<u64>,
    ...
    /// The `VolumeSerialNumber` property contains the serial number of the
    /// volume.
...
/// Query logical disks via WMI (Windows only)
#[cfg(windows)]
