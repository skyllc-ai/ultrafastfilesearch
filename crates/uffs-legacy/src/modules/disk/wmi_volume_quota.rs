...
pub struct Win32VolumeQuota {
    ...
    /// The `Setting` property contains the WMI path to the associated quota
    /// setting For example: `Win32_QuotaSetting.VolumePath="F:\\"`
    Setting: String,
    ...
    /// hierarchy. For `Win32_VolumeQuota`, this might be
    /// `CIM_ElementSetting` or `Win32_Volume`.
    ...
    /// class. It shows which class `Win32_VolumeQuota` inherits from.
...
/// Query volume quotas via WMI (Windows only)
#[cfg(windows)]
pub fn query_volume_quota() -> Result<Vec<Win32VolumeQuota>, UFFSError> {
    ...
    let results: Vec<Win32VolumeQuota> = wmi_con
        .raw_query(query)
