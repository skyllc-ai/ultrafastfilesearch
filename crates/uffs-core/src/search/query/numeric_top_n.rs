...
fn resolve_chunk<D: AsRef<DriveCompactIndex>>(
    ...
    for &(drive_idx, rec_idx, _) in chunk {
        ...
        // Skip nameless records on the LOSSLESS byte length, not the lossy
        // `&str`: an ill-formed (surrogate) name has bytes but an empty `&str`
        // view, and must still be enumerated (WI-4.4 — it can't hide).
        if rec.name_len == 0 {
            continue;
        }
