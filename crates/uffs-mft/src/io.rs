...
// Re-export MftRecordMerger from parse module (now cross-platform)
...
///
/// This struct provides accurate timing for each phase of MFT reading,
/// allowing apples-to-apples comparison with C++ `--benchmark-index` output.
///
/// Note: Since I/O and parsing can overlap in pipelined modes, the sum of
/// `io_ns + parse_ns + merge_ns` may exceed `wall_ns`. This is expected and
/// provides insight into the overlap efficiency.
...
mod tests {
    ...
    // =========================================================================
    // Tests for optimal settings propagation
    // =========================================================================
    ...
    fn test_parallel_mft_reader_uses_optimal_chunk_size() {
        ...
        // Create a minimal extent map for testing
        let extent_map = MftExtentMap::contiguous(100, 1024 * 1024, 4096, 1024);

        // Test NVMe uses 4MB chunk size
        let nvme_reader =
        ...
        // Test Unknown uses 1MB chunk size (conservative)
        let unknown_reader =
    ...
    fn test_drive_type_stored_in_reader() {
        ...
        // Verify drive type is stored correctly for later use by I/O methods
        for drive_type in [
            DriveType::Nvme,
            DriveType::Ssd,
            DriveType::Hdd,
    ...
    /// Test that verifies the optimal settings are used when None is passed.
    /// This simulates what happens when MftReader uses default settings.
    #[test]
    fn test_optimal_defaults_when_none_passed() {
        use crate::platform::DriveType;

        // Simulate the unwrap_or_else pattern used in I/O methods
    ...
    /// Test that parallel parsing is auto-enabled only for NVMe
    #[test]
    fn test_parallel_parsing_auto_detection() {
        use crate::platform::DriveType;

        // Simulate the unwrap_or_else pattern used in reader.rs
