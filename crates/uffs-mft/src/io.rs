...
fn parse_file_name(
    ...
    // Extract parent FRS
    result.parent_frs = file_reference_to_frs(fn_attr.parent_directory);
    ...
    // Get file size from $FILE_NAME if not already set
    if result.size == 0 && fn_attr.data_size > 0 {
        result.size = fn_attr.data_size as u64;
    }
}
