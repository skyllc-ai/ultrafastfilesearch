...
pub(crate) async fn cmd_benchmark_index_lean(
    ...
    } else if matches!(drive_type, uffs_mft::platform::DriveType::Hdd | uffs_mft::platform::DriveType::Removable | uffs_mft::platform::DriveType::Virtual) {
    ...
    for &drive in &drives {
        ...
        let extents = handle.get_mft_extents().unwrap_or_else(|e| {
            ...
            vec![MftExtent {
                ...
                lcn: uffs_mft::platform::Lcn::new(volume_data.mft_start_lcn.cast_signed()),
            }]
        });
