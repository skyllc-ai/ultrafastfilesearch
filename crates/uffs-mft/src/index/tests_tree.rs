...
fn compute_tree_metrics_simple() {
    ...
    let rec = index.get_or_create(file3_frs.into());
    rec.first_name.name = IndexNameRef::new(offset, 9, true, IndexNameRef::NO_EXTENSION);
    rec.first_name.parent_frs = Into::into(root_frs);
    rec.first_stream.size = SizeInfo {
        length: 500,
    ...
    index.add_child_entry(Into::into(root_frs), Into::into(dir1_frs), 0);
    index.add_child_entry(Into::into(root_frs), Into::into(file3_frs), 0);
    index.add_child_entry(Into::into(dir1_frs), Into::into(file1_frs), 0);
    index.add_child_entry(Into::into(dir1_frs), Into::into(file2_frs), 0);
    ...
    let file1_idx = index.frs_to_idx_opt(file1_frs.into()).unwrap();
    ...
    let file2_idx = index.frs_to_idx_opt(file2_frs.into()).unwrap();
    ...
    let file3_idx = index.frs_to_idx_opt(file3_frs.into()).unwrap();
    ...
    // dir1 = 1 + 1 + 1 = 3
    let dir1_idx = index.frs_to_idx_opt(dir1_frs.into()).unwrap();
    ...
    // root = 1 + 3 + 1 = 5
    let root_idx = index.frs_to_idx_opt(root_frs.into()).unwrap();
...
fn compute_tree_metrics_deep_tree() {
    ...
    let rec = index.get_or_create(dir2_frs.into());
    rec.stdinfo.set_directory(true);
    rec.first_name.name = IndexNameRef::new(offset, 4, true, IndexNameRef::NO_EXTENSION);
    rec.first_name.parent_frs = Into::into(dir1_frs);
    ...
    let rec = index.get_or_create(dir3_frs.into());
    rec.stdinfo.set_directory(true);
    rec.first_name.name = IndexNameRef::new(offset, 4, true, IndexNameRef::NO_EXTENSION);
    rec.first_name.parent_frs = Into::into(dir2_frs);
    ...
    rec.first_name.parent_frs = Into::into(dir3_frs);
    rec.first_stream.size = SizeInfo {
        length: 1000,
        allocated: 4096,
    };
    ...
    // Add child entries (required for tree metrics algorithm)
    index.add_child_entry(Into::into(root_frs), Into::into(dir1_frs), 0);
    index.add_child_entry(Into::into(dir1_frs), Into::into(dir2_frs), 0);
    index.add_child_entry(Into::into(dir2_frs), Into::into(dir3_frs), 0);
    index.add_child_entry(Into::into(dir3_frs), Into::into(file_frs), 0);
    ...
    let file_idx = index.frs_to_idx_opt(file_frs.into()).unwrap();
    ...
    let dir3_idx = index.frs_to_idx_opt(dir3_frs.into()).unwrap();
    ...
    let dir2_idx = index.frs_to_idx_opt(dir2_frs.into()).unwrap();
...
fn compute_tree_metrics_performance() {
    ...
    let dir0_idx = index.frs_to_idx_opt(100.into()).unwrap();
