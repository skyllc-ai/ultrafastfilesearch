//! Compatibility stub for the old `dump_mft_records` binary location.
//!
...
//! `uffs dump_mft_records` (or reference the old path) receive a clear and
...
fn main() {
    eprintln!(
        "dump_mft_records has moved to the uffs-diag crate.\n\
	     Please run it via `cargo run -p uffs-diag --bin dump_mft_records -- <args>`",
    );
}
